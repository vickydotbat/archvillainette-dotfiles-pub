#!/usr/bin/python3
# Source: https://gist.github.com/runningnak3d/1b9e2df0239f7326965d918829828bd1
...
#
BAR_CHARACTERS = dict([
    (000, '%{F#2E3440}▁%{F-}'),  # Zero output

    (BAR_FACTOR * 1, '%{F#3B4252}▁%{F-}'),
    (BAR_FACTOR * 2, '%{F#434C5E}▂%{F-}'),
    (BAR_FACTOR * 3, '%{F#4C566A}▃%{F-}'),
    (BAR_FACTOR * 4, '%{F#5e81ac}▄%{F-}'),
    (BAR_FACTOR * 5, '%{F#81a1c1}▅%{F-}'),
    (BAR_FACTOR * 6, '%{F#88c0d0}▆%{F-}'),
    (BAR_FACTOR * 7, '%{F#8fbcbb}█%{F-}'),

    (100, '%{F#8fbcbb}█%{F-}'),  # Highest output
])
...
def createCavaConfig():
    ...
    config.set('general', 'framerate', str(30))
